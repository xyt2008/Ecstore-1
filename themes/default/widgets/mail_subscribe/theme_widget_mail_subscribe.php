...
function theme_widget_mail_subscribe(&$setting,&$smarty){
    ...
    subscribe_new_foo($lv, $iNodeId, $limit, $setting['showallart'], $oAN, $oMAI, $tmp['child'], $setting);
    $html = array();

    subscribe_new_show($smarty, $tmp['child'], $setting, $html, 0, $limit);
    if( !$setting['shownode'] ) {
...
function subscribe_new_foo($lv=1, $iNodeId=1, $limit, $showallart, $oAN, $oMAI, &$tmp, $setting) {
    if($lv<0)return;
    ...
    if(is_array($aNodes)) {
        foreach ($aNodes as $val) {
            ...
            subscribe_new_foo(($lv-1), $val['node_id'], $limit, $showallart, $oAN, $oMAI, $tmp['child'][$val['node_id']], $setting);
...
function subscribe_new_show(&$smarty, $tmp, $setting, &$html, $lv=0, &$limit) {
    if($setting['shownode'] && $lv!=0) {
        ...
        $html[] = subscribe_new_html($lv, $url, $tmp['info']['node_name'],$typeClass);
    }
    ...
    if($tmp['article']) {
        ...
        foreach ($tmp['article'] as $idx=>$row) {
            ...
            if($limit>0){
                if($idx == 0){
                    $html[$key] = "<li class='article-index-list'><ul>".subscribe_new_html($tmp_lv, $url, $article_title,'article-index');
                }elseif($idx == ($len-1)){
                    $html[$key] = subscribe_new_html($tmp_lv, $url, $article_title,'article-index')."</ul></li>";
                }else{
                    $html[$key] = subscribe_new_html($tmp_lv, $url, $article_title,'article-index');
                }
            }
    ...
    if($tmp['child']) {
        foreach ($tmp['child'] as $row) {
            subscribe_new_show($smarty, $row, $setting, $html, $lv+1, $limit);
        }
    }
}

function subscribe_new_html($lv, $url, $name,$type) {
    return <<<EOF
